...
export default function CreateServiceWizard({
    ...
    const fetchProjects = useCallback(async () => {
        ...
        try {
            const res = await fetch(`http://localhost:8000/projects?team_id=${teamId}`, { headers: { Authorization: `Bearer ${token}` } });
            if (res.ok) {
    ...
    const checkGitHubConnection = useCallback(async () => {
        try {
            const res = await fetch("http://localhost:8000/github/connection", { headers: { Authorization: `Bearer ${token}` } });
            if (res.ok) {
    ...
    const fetchRepos = useCallback(async () => {
        ...
        try {
            const res = await fetch("http://localhost:8000/github/repos", { headers: { Authorization: `Bearer ${token}` } });
            if (res.ok) {
                const d = await res.json();
                setRepos(d || []);
            }
    ...
    const fetchBranches = useCallback(async (owner: string, repo: string) => {
        ...
        try {
            const res = await fetch(`http://localhost:8000/github/branches?owner=${owner}&repo=${repo}`, { headers: { Authorization: `Bearer ${token}` } });
            if (res.ok) {
                const d = await res.json();
                setBranches(d || []);
            }
    ...
    const handleGitHubConnect = async () => {
        try {
            const res = await fetch("http://localhost:8000/github/auth", { headers: { Authorization: `Bearer ${token}` } });
            if (res.ok) {
                const d = await res.json();
    ...
    useEffect(() => {
        const handleMessage = async (event: MessageEvent) => {
            ...
            if (event.data.type === "GITHUB_CONNECTED") {
                ...
                try {
                    const res = await fetch("http://localhost:8000/github/connect", {
                        method: "POST",
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const sRes = await fetch("http://localhost:8000/services", {
                method: "POST",
                headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                body: JSON.stringify({
